...
class ResultIndex(object):
    ...
    def nxsEntryPoint(self):
        return PosixPath(f'/analyses/MCResult{self.resultIndex}')

def storeKVPairs(filename:Path, path:PosixPath, pairs:Iterable) -> None:
...
def loadKVPairs(filename:Path, path:PosixPath, keys:Iterable) -> None:
    assert filename is not None
    assert path is not None
...
def loadKV(filename:Path, path:PosixPath, datatype=None, default=None, dbg=False): # outputs any hdf5 value type
